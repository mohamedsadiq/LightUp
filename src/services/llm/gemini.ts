...
Instructions: Build on your previous analysis/explanation of this content. If the question asks for "more" or "other" aspects (like "what else is strange/unusual/problematic"), provide genuinely new insights you haven't covered yet. Avoid repeating previous points unless directly relevant to the new question.`;
    ...
    try {
        ...
        if (!settings.geminiApiKey || settings.geminiApiKey.trim() === '') {
            throw new Error('❌ Gemini API key is missing. Please add your API key in the extension settings.');
        }

        const apiKey = settings.geminiApiKey.trim();

        if (apiKey.length < 30) {
            throw new Error('❌ Gemini API key appears to be incomplete. Please verify your complete API key.');
        }

        if (apiKey.includes(' ') || apiKey.includes('\n') || apiKey.includes('\t')) {
            throw new Error('❌ Gemini API key contains invalid characters (spaces, tabs, or line breaks). Please copy the key carefully without extra whitespace.');
        }

        const modelName = settings.geminiModel || "gemini-2.0-flash"; // Default to 2.0 Flash for reliability
        console.log(`[Gemini] Using model: ${modelName}, API key validated`);
